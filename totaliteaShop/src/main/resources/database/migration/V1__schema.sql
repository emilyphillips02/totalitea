...
CREATE TABLE totalitea.orders (
                                  id BIGSERIAL PRIMARY KEY,
                                  user_id INT REFERENCES totalitea.users(id) ON DELETE CASCADE,
...
CREATE TABLE totalitea.order_items (
                                       id BIGSERIAL PRIMARY KEY,
                                       order_id INT REFERENCES totalitea.orders(id) ON DELETE CASCADE,
                                       product_id INT REFERENCES totalitea.products(id) ON DELETE CASCADE,
...
-- Indexes
CREATE INDEX totalitea.idx_products_name_supplier_type
    ON totalitea.products (name, supplier, type);

CREATE INDEX totalitea.idx_orders_user
    ON totalitea.orders (user_id);

CREATE INDEX totalitea.idx_order_items_order
